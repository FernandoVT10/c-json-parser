...
int string_cmp_text(String *str, const char *text) {
    da_append(str, '\0');
    int res = strcmp(str->items, text);
    str->count--;
    return res;
}

char* string_dump(String *str) {
    return strndup(str->items, str->count);
}
...
const char *text_format(const char *format, ...) {
    static char buffer[MAX_TEXT_BUFFER_SIZE];

    bzero(buffer, MAX_TEXT_BUFFER_SIZE);

    va_list args;
    va_start(args, format);
    vsnprintf(buffer, MAX_TEXT_BUFFER_SIZE, format, args);
    va_end(args);
